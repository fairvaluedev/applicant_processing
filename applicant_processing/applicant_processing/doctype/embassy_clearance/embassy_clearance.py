...
class EmbassyClearance(Document):
	def validate(self):
		if self.dsr:
			...
			if not self.destination_country and hasattr(dsr_doc, "destination_country"):
				self.destination_country = dsr_doc.destination_country
		elif not self.full_name:
			self.full_name = f"{self.first_name or ''} {self.last_name or ''}".strip()

	def on_update(self):
		if self.dsr:
			frappe.db.set_value("DSR", self.dsr, "embassy_status", self.status, update_modified=False)
